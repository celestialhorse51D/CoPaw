...
>      - uvパス（いずれか一つ、インストール場所に応じて選択。`uv`が利用不可の場合に記入）：通常`%USERPROFILE%\.local\uv\bin`、`%USERPROFILE%\AppData\Local\uv\bin`、またはPythonインストールディレクトリの`Scripts`フォルダ
...
>   - **uvを手動でインストール**： [GitHub Release](https://github.com/astral-sh/uv/releases) を参照し、`uv.exe` を `%USERPROFILE%\.local\uv\bin` または `%USERPROFILE%\AppData\Local\uv\bin` に配置。または Python がインストールされていることを確認し、`python -m pip install -U uv` を実行。
