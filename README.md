...
>      - uv path (choose one based on installation location; use if `uv` fails): Typically `%USERPROFILE%\.local\uv\bin`, `%USERPROFILE%\AppData\Local\uv\bin`, or the `Scripts` folder within your Python installation directory
